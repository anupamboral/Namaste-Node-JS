...
//! ⁡⁢⁣⁡⁢⁣⁢⁡⁢⁣⁡⁢⁣⁡⁢⁣⁢Season 2 - Episode - 01 - Microservices vs Monolith
//* we will learn in the industry how projects are built.
//* in the industry, to build a project always a model is being followed that is known as waterfall model(SDLC- software development life cycle).Let's learn about this model.
//* see image -images\waterfall model.jpg
//* to develop a project , it has to go through all of these steps.

//* 1.Requirements:- first what will be the requirements,features will be decided by the product /project managers with the designers
//* 2.Design:- Senior engineers , engineering managers creates a design document in this phrase, they decide the architecture(monolith or microservice) , tech stack,HLD,LLD in this phrase.
//* 3.Development:- SDE1, SDE2 write code for the product and also write unit test cases in this phrase.
//* 4.Testing :- SDET(software development engineer testing) write manual or automation test cases to test the application in this phrase.
//* 5.Deployment:- Devops(if present) team manages the deployment.
//* 6.Maintenance:- if we want to maintain or add any new feature then this whole process will repeat from first step.

//* in the industry to create any application mainly these two architectures are followed
//? Monolith v/s Microservice architecture
//*see image - images\Monolith vs Microservices.jpg
//* In Monolith architecture , we have a one big single project, it does everything , it has database connection,backend, frontend, analytics , emails, authentication , every thing is written in a single code repository.
//* In Microservice architecture,here "service" / application /project are inter changeable used words so they mean the same. So in this architecture, for frontend there can be a different service/application, for backend there can be different service/application , we can create separate applications/services for analytics, database, form , emails,notifications, admin dashboard.And these all services will communicate with each other. In a large company, there will separate teams for each service.

//?Difference between Monolith and Microservice architecture
//*1.Code Repo :-In Monolith there is a single code repository where all the developers are working suppose like 20-30 developers are working on the single code repository  and in microservices  there are some developers working on one service other developers working on one service like a small group of developers are working on individual services ,so what happens is single code repo and in microservices multiple code repositories.
//* Dev Speed:- Development speed is slower in monolith  because a lot of people are working on the same repository code reviews become slower , sometimes you know it becomes tough for execution also you know when you take a very big project into your system and try to execute it so it the development speed becomes low right and you know what happens suppose if you are running that application you have to compile the whole project verb work it see the more the code the slower it will run isn't it so the depth speed is compromised a little when you are using monolith in microservices its very fast right in Microservices you can build it parallely also see you can build parallel monolith also but in microservices it becomes very easier that a front end team is managing their own project a backend team is managing their own back end and they are building things up parallely in monoliths sometimes it is difficult to maintain that combination so dev speed according to me is faster in microservices slower in monolith.
//* 3.Scalability:- When you are building a small project when you are a small company, so scalability is not a challenge in Monolith but as soon as you are very big company like Uber , Uber is a very very big company but even at a big scale then it becomes very tough to scale monolith architecture very tough ,why? because your code repository is growing day by day every day there are so many lines of code which is being pushed so maintaining that big repository is a mess but suppose if you are creating small small microservices so it becomes easier to manage those small microservices and scale those services independently so let me tell you an example suppose if you have a front end microservice there is a back end microservice so solving the challenges in that small project is and scaling that repository is good right scaling and monolith architecture is tough it is very tedious thing right and architects spend a lot of time doing that and figuring out ways to do that so in microservices suppose you have a analytics engine and you want to scale that analytics engine you can do it independently alright so scalability is good in microservices bad in monolith
//*4.Deployment :In monolith you just have to do a single deployment your whole application is deployed in microservice you have to deploy each and every application separately now this can be a pro and this can be a con also let me tell you why suppose if you have to deploy suppose if you just have to deploy a new feature on front end right front end now in Monolith architecture you will have to deploy the whole project to just deploy front end suppose if you want to make a one line change in your front end code but it cannot happen unless the whole code is deployed onto the server  deploying the whole code is a tedious task it takes time and a lot of process and goes through testing and Whatnot right even if it is small change one line change in the front end still the whole testing process and everything is followed so it becomes tough in monolith but in microservice suppose if you want to just make up one line change in front end you can just deploy your front end application- front end microservice and all the other microservice works in the same way right so it is an advantage in microservice but this can be a disadvantage also see when Microservices are communicating with each other if the back end microservice version is not aligned with the front end version then there can be a mismatch ,  suppose the front end is talking to some other version of back end then there can be a mismatch otherwise if you don't worry about that if the coordination is good then it's not a problem  and  when it happens in Micro Services so whenever there is a ground breaking changes so all the subscribers are already notified so suppose if there are 500 projects using some apis , so you always keep your apis backward compatible so what I am trying to say is there is a way of managing this deployment issues right version mismatch issues in your microservice architecture ,in my opinion I would always prefer micro services because see I have seen cases where you just have to make a small change and you can deploy it anytime because its your micro service and you don't have to take a lot of permissions and it becomes very easier for you to do continuous deployment you have you can set up a CI-CD pipeline in your own separate projects it becomes very easy in the microservice architecture

//* 5.Tech Stack:- In Monolith you are restricted with the tech stack, suppose in monolith it's 1 code base now suppose if you are writing some front end you can choose a react , suppose if you chose react or you chose angular then you are sticking to it , everything that you build you have to stick to react and  in Micro Service architecture suppose if you are building a admin dashboard you can build that in react, if you are building a student dashboard you can build it next js ,so let me give you an example also in NamasteDev.com, we have built the admin dashboard using react the website that you use namasteDev.com it is built on nextJs ,so because they are two different microservices,  in microservice architecture  you independently can use different tech stack suppose if you want to build your analytics using go lang you can build that suppose if your back end is written in Java you can build that if it is a separate microservice, but in monolith if you are using Java stack then you have to stick to Java for sending emails you have to stick to Java for analytics dashboard you have to stick to Java for everything , but in microservice you feel free to choose whatever tech stack you are choosing for different applications , in very big companies generally you would see that some microservices are written in Java some microservices are written in node js some Micro Service are written in python some microservice are written in Golang and its a good thing , it use the tech stack which is best suited for that particular micro service , thats why Micro Service architecture
// *6.Infra Cost:- Infra Cost is a little lower in monolith than microservice because see the more the applications the more the infra is required, but see infra basically depends on the traffic most of the times but again its the way of how devops works but in a nutshell monolith has a easier infra cost , its like you deploy the single project on a server , how many servers you want to deploy but in microservices you have to manage the infra according to your microservices , so you need three servers for front end or four servers for back end and you can customize it on your own  but that's how things are but infra cost will be slightly higher in microservices because it is a separate microservice need a separate server separate teams take their own servers and all that stuff right even for testing also suppose I am a developer I want to test my particular microservice so I will take a new staging environment a pre-production environment and do that right so infra cost is a little higher for micro services .
// *7.complexity :-When you are building a very small project then complexity is tough in microservices because see you just have a small project you are managing like five services for it so its tough in microservice but as you grow  complexity increases in  monolith .So much code, so many files, folder this that so it becomes very messy , so complexity is tough for large project in monolith complexity is easy in Microservice when you are building a large application.
// *8.Fault isolation:- Suppose  if one line of code in back end is faulty in Monolith then the whole project breaks suppose if a small thing breaks in some piece of code in Monolith so the whole project can crash but in microservice it just crashes only that microservice suppose if there is a breaking change ,so only that microservice is affected actually the whole system is affected but then still that whole microservices will go down not everything will go down for example suppose if there is a issue in Analytics so only analytics service is going down not everything is going down right so fault isolation is there in microservice in monolith it is tough
// *9.Testing :- Microservices can be tested independently, Write all the services can have their own test cases in monolith you write integration test cases for different things but let me let me tell you my observation writing test cases in Monolith was a little easier because you can write end to end test cases for the whole user flow because its just one project you can write the end to end test cases all throughout but writing end to end test cases in microservices the whole journey of user in microservices becomes very very tough because these are different services all these services are having different kind of apis so talking writing end to end test cases is tough in microservices so here in testing monolith is a little easier
//*10.Ownership :-Different teams takes the Ownership of different microservices in monolith there is a central ownership  and there are few people in the companies which decide that how will the deployment work and all that stuff, in microservices there is a owner of that small team who is managing that Microservice.
// *11.Maintenance and Revamp:- It is easier to maintain micro services and revamps also suppose in maintenance and revamp suppose if you want to change the whole front end , so it is easier to change the microservice , you can just create a new microservices with a new front end  and you can revamp it but it becomes tough to do the same thing in the monolith architecture.
// *12.Debugging:- Now debugging can be tough in Monolith it can be tough in microservice also because if something breaks then you will have to find out which microservices  is breaking it and there can be blame games also I have seen in the companies suppose if something goes wrong one team is blaming the other team they are saying that your service is faulty that the saying my service is good and  your service is faulty so there can be a blame game , in Monolith debugging can be slightly easier because it's a whole 1 code repository if it fails its easier to debug which file which code is breaking ,its slightly easier.
//*13.Dev experience :- I've worked with monolith, i've worked with Micro Service I love microservice architecture and I've try to break my services down into smaller services so that they can independently scale, independently have their own deployment cycle, independently dev efforts and all that stuff I like to do that so this is what my opinion on monolith and microservices.
//? What happens in Namastedev.com?
//* So let me tell you there is one microservices which we call it as student web so basically this is the student web this is the front end application which you are using right now to watch these videos this is the student web and it has its own text app it is written in next js got it we have a admin dashboard which is known as admin web so it is a admin dashboard and it is written in react js we have another these are separate microservice all together these are separate applications now we have a back end application right back end application this is written in node js and this backend application talks to the admin talks to the student web now we have these three microservice and we are also building 1 more microservice which will be for the mobile application right which will be for the student student app mobile app right student mobile app ok so for mobile app and we will be using react native to build this right so what I am trying to say there are four microservice which we are using it now you can ask me how do you send emails so basically all the code of sending emails is written inside back end all the analytics is written inside back end right but as our project will grow we might split this back end microservice into smaller services for Analytics smaller services for sending notifications but we are not very big of a company so if we become very big then we will small slowly slowly you know when the company starts they start with just 1 or 2 microservice then they start building new new microservice as the application grows and as the need grows while you are watching this project while we are watching this video right we might have started planning to build new microservices in namasteDev but right now we are just working with these two microservices.
